...
$("#drpdownUOM").on('change', function () {
    ...
    $.ajax({
      ...
      url: "http://localhost:8091/admin/unit/scbu/deleteDonutdetails",
...
function saveTable() {
    ...
    $.ajax({
        ...
        url: "http://localhost:8091/admin/unit/scbu/addDonutDetails",
    }).done(function (data) {
      var status = data;
      if (status == "success") {
  ...
  function getDate() {
    ...
    { 
        var unitName = {unitName: $('#drpdownUnitName').val() ,donutType: $('#drpdownUOM').val()}
     ...
     $.ajax({
       ...
       method: "POST",
        ...
        for (const val of data) {
            ...
            ittrdata += '<td><select class="form-control positive">'
