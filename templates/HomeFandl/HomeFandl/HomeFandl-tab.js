...
function saveTable() {
    ...
    for ( i =0; i <= tabledata.rows.length-1; i++) {
        ...
        const dataObj = {
            ...
            Fuel: $(numberRow).find(".drpDwndatatype3").val(),
            Loss: $(numberRow).find(".drpDwndatatype3").val()
        }
  ...
  function getDate() {
    ...
    //   }).done(function (data) {
        ...
        for (const val of tabledata) {
          ...
          ittrdata += '<td><input class="form-control inputColumnName" id="lebal" type="text" placeholder="Enter Column Name" value="'+ val['lebal'] +'"/></td>';
          ittrdata += '<td><select class="form-control js-example-basic-single drpDwndatatype3" type="text" placeholder=""></select></td>';
          ittrdata += '<td><select class="form-control js-example-basic-single drpDwndatatype4" type="text" placeholder=""></select></td>';
          ittrdata += '</tr>';
